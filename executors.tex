...
\begin{enumerate}
\item\label{it:library-call-issue} Exhaustive exploration of external library calls may lead to an exponential explosion of states, preventing the analysis from reaching the interesting portions of the code. %The problem is exacerbated by any interactions with the environment (files, network, etc.) made in the library.
...
\noindent A fundamental idea that has been proposed in literature to cope with the above issues and make symbolic execution feasible in practice is to mix concrete and symbolic execution: this is dubbed {\em concolic execution}, where the term concolic is a portmanteau of the words ``concrete'' and ``symbolic'' (Figure~\ref{fig:concrete-symbolic}). This general principle has been explored along different angles, discussed in the remainder of this section. %We discuss advantages and disadvantages of the different approaches, deferring an additional discussion of issue~(\ref{it:smt-solver-issue}) to Section~\ref{se:constraint-solving}.

\myparagraph{Dynamic Symbolic Execution} One popular concolic execution approach, known as {\em dynamic symbolic execution} or {\em dynamic test generation}~\cite{DART-PLDI05}, is to have concrete execution drive symbolic execution. This technique can be very effective in mitigating issues (\ref{it:library-call-issue}), (\ref{it:third-party-issue}), and (\ref{it:smt-solver-issue}). In addition to the symbolic store and the path constraints, the execution engine maintains a concrete store $\sigma_c$ as well. After choosing an arbitrary input to begin with, it executes the program both concretely and symbolically by simultaneously updating the two stores and the path constraints. Whenever the concrete execution takes a branch, the symbolic execution is directed toward the same branch and the constraints extracted from the branch condition are added to the current set of path constraints. In other words, the symbolic execution is driven by a specific concrete execution. A consequence of this approach is that the symbolic engine does not need to invoke the constraint solver to decide whether a branch condition is (un)satisfiable, since this is directly tested by the concrete execution. In order to explore different paths, the path conditions given by one or more branches can be negated and the SMT solver invoked to find a satisfying assignment for the new constraints, i.e., to generate a new input. This strategy can be repeated as much as needed to achieve the desired coverage. \mynote{fwd refs for 1, 2, 3?}
}
...
\mytempedit{{\sc \stwoe}~\cite{CKC-TOCS12} takes a different approach to mixing symbolic and concrete execution based on the observation that one may want to explore only some components of a software stack in full, but not to care about others. {\em Selective symbolic execution} carefully interleaves portions of code that are concretely run with fully symbolic phases, while keeping the exploration meaningful.

Going from the concrete to the symbolic domain is easy: the called function is executed both concretely and symbolically, with the result from the first execution simply returned to the concrete domain, while  symbolic execution explores the component of interest in full by turning the arguments for the call into purely symbolic ones.

When calling instead from the symbolic to the concrete domain, the arguments for the call are concretized and added to the path constraints, as the return value and any possible side effects are implicitly tied to the choice of values for the arguments. This results however in future symbolic execution paths possibly being lost because of (a) the values chosen for the arguments or (b) the constrained return value and side effects. Constraints of kind (a) and (b) are thus marked by the engine as soft, and whenever a branch in the symbolic domain is made inoperative by such a constraint, the execution can backtrack and a different choice of concrete values can be made.
...
\mytempedit{
  Another interesting search strategy is the {\em shortest-distance symbolic execution} (SDSE) heuristic presented by~\cite{MPF-SAS11}. This work does not target coverage but aims at identifying program inputs that trigger the execution of a specific target point in a program. However, similarly to other coverage-based strategies, it is based on a metric for evaluating the shortest distance to a target point. This is computed as the length of the shortest path in the inter-procedural control-flow graph (ICFG). Paths with the shortest distance are prioritized by the engine. 
}
...
Symbolic backward execution (SBE)~\cite{CFS-PLDI09,DA-ASE14} is a variant of symbolic execution in which the exploration proceeds from a target point to an entry point of a program. The analysis is thus performed backward, i.e., in the reverse direction compared to canonical (forward) symbolic execution.
The main purpose of this approach is typically to identify an instance of the program inputs that can trigger the execution of a specific line of code (e.g., an {\tt assert} statement or a {\tt throw} instruction). %  

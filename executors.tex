...
A crucial requirement for the reversed exploration in both SBE and CCBSE is the availability of the {\em inter-procedural control-flow graph}, which by providing a whole-program control-flow graph allows determining the call sites for the functions that are involved in the exploration. Unfortunately, constructing such a graph can be quite challenging in practice\iffullver{ (see, e.g., the discussion about CFG reconstruction in Section~\ref{se:symbolic-binary}).}{\mynote{cite supplementary material or some other paper}.} Moreover, a function may have many possible call sites, making the exploration performed by SBE still very expensive. On the other hand, some practical advantages can arise when the constraints are collected in the reverse direction. We will further discuss these benefits in Section~\ref{se:constraint-solving}. 

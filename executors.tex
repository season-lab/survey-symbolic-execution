...
(1) {\em From concrete to symbolic}: the arguments of B are made symbolic and B is explored symbolically. B is also executed concretely and its concrete result is returned to A, which resumes concretely. 
(2) {\em From symbolic to concrete}: the arguments of B are concretized and B is executed concretely. The concretization may affect the paths taken after returning in A, possibly leading to missed branches. To remedy this, for each concretized argument of B a {\em soft} constraint is added to the path constraints, keeping track which concrete value was used for that argument. Whenever a branch after returning to A is made inoperative by a soft constraint, the execution backtracks and a different choice of arguments for B is attempted. To guide the re-concretization, {\sc \stwoe} collects the branch conditions during the concrete execution of B, and chooses the concrete values so that they enable a different concrete execution path when B is re-executed.

...
\mytempedit{Based on the observation that a large number of paths can be considered ``equivalent'' since the symbolic expressions describing the output are the same, \cite{QNR13} proposes a path partitioning approach where two program paths are placed in the same partition if they have the same relevant slice with respect to the program output. A relevant slice is the transitive closure of dynamic data, control and \mynote{[D] potential?} potential dependencies. Paths are partitioned on-the-fly during their exploration, computing a concise semantic signature for a program, which describes all the different symbolic expressions that the output can assume along different paths.}

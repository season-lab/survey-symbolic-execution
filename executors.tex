...
(2) {\em From symbolic to concrete and back}: the arguments of B are concretized, B is executed concretely, and then execution resumes symbolically in A. Note that the concretization may affect the paths taken after returning in A, possibly leading to missed branches. To remedy this, a {\em soft} constraint is added to the path constraints for each concretized argument of B, keeping track of the concrete value used for that argument. A soft constraint is also added for the concrete return value of B. Whenever a branch after returning to A is made inoperative by a soft constraint, the execution backtracks and a different choice of arguments for B is attempted. To guide re-concretization of B's arguments, {\sc \stwoe} collects the branch conditions during the concrete execution of B, and chooses the concrete values so that they enable a different concrete execution path in B.

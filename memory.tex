...
\item {\em Leveraging language-specific features.} \mynote{[D] assumption can be used to?} Differently from languages with unrestricted memory access, such as C or assembly, some object-oriented languages restrict references to either
be null or point to previously allocated objects, rather than to any generic memory location. This assumption can significantly reduce the exploration space in symbolic execution, as discussed in Section~\ref{ss:complex-objects}.
\end{itemize}}
...
\mytempedit{ DART concretizes a reference of type {\tt T*} to either {\tt NULL}, or to the address of a newly allocated object of {\tt sizeof(T)} bytes. The choice is made randomly. If {\tt T} is a structure, the same concretization approach is recursively applied to all fields of the object. [...]}

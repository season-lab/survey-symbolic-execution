...
\boxedexample{Consider the scenario in Figure~\ref{fi:fully-symbolic-memory} (a). The write operation at line X affects either $a[0]$ or $a[1]$, depending on the unknown value of array index $i$. Execution is therefore forked to explicitly consider both possible scenarios. The path constraints for the forked states encode the assumption made on the value of $i$. }

\item {\em ITE formulas.} 

\end{itemize}

\noindent In general, a symbolic address may reference any cell in memory, making the approaches described above intractable. Fortunately, in many practical cases the set of possible addresses a memory operation may reference is small~\cite{BITBLAZE-ICISS08}, as in the example shown in Figure~\ref{fi:fully-symbolic-memory} where $a[i]=5$ can write to either $a[0]$ or $a[1]$. In these cases, .

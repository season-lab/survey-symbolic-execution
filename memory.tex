...
\item {\em Leveraging language-specific features.} Differently from languages with unrestricted memory access, such as C or assembly, some object-oriented languages restrict references to either
be null or point to previously allocated objects, rather than to any generic memory location. This assumption can significantly reduce the exploration space in symbolic execution, as discussed in Section~\ref{ss:address-concretization} and Section~\ref{ss:complex-objects}.
...
which handle memory initialization by concretizing a reference of type {\tt T*} either to {\tt NULL}, or to the address of a newly allocated object of {\tt sizeof(T)} bytes. DART makes the choice randomly, while CUTE first tries {\tt NULL}, and then, in a subsequent execution, a concrete address. If {\tt T} is a structure, the same concretization approach is recursively applied to all fields of a pointed object. Since memory addresses (e.g., returned by {\tt malloc}) may non-deterministically change at different concrete executions, CREST uses {\em logical addresses}\mynote{troppo sintetico?} in symbolic formulas to maintain consistency across different runs. Another reason for concretization is due to limitations in constraint handling: for instance, CUTE is capable of reasoning only about equality constraints for pointers, as they can be solved efficiently, and resort to concretization for general symbolic references.
}

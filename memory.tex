...
\item {\em Heap modeling.} Another effective technique is to corset the exploration to states where pointers are restricted to be either null, or point to previously heap-allocated objects, rather than to any generic memory location (\ref{ss:address-concretization} and Section~\ref{ss:complex-objects}).

...
\mytempedit{
A recent orthogonal approach that can help reduce the number of paths to check is presented in~\cite{SSJ-NFM15}. The main intuition is that even if the solver is used for reasoning over a single path, it will often ends up reasoning over statements shared by many paths. ~\cite{SSJ-NFM15} exploits this observation by extracting a minimal {\em unsat core} from each path that is proved to be unsatisfiable. 
% This can can be seen as a set of learned conflicts and 
This is obtained by removing as many as possible statements from the path while keeping unsatisfiability. An engine can exploit unsat cores to discard paths that share common (unsatisfiable) statements.
}
...
In early works, summaries could be generated only for loops that update symbolic variables across the iterations by adding a constant, non-zero amount. Moreover, it was not possible to handle nested loops as well as {\em multi-path loops}, i.e., loops that contain one or more branches. In the last few years, the latter scenario has been effectively tackled by Proteus~\cite{XX-FSE16}. %by S-Looper~\cite{XX-ISSTA15} and later 

...
In early works, summaries could be generated only for loops that update symbolic variables across iterations by adding a constant, non-zero amount. Moreover, they could not handle nested loops and {\em multi-path loops}, i.e., loops that contain one or more branches. Recently, the latter scenario has been effectively tackled by Proteus~\cite{XX-FSE16}. %by S-Looper~\cite{XX-ISSTA15} and later 

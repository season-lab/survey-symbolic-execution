...
\noindent \mytempedit{Typical types of preconditions considered in symbolic execution are: {\em Known length} (i.e., the size of a buffer is known), {\em Known prefix} (i.e., a buffer has a known prefix), and {\em Fully known} (i.e., the content of a buffer is fully concrete). These preconditions can be very common when dealing with code that operates over input data with well-defined structure, such as string parsers or packet processing tools. 
...
\begin{shaded}\mytempedit{\noindent{\bf\small Example.} Consider the code shown below and assume that {\tt in} points to the input buffer, while {\tt exp} points to the expected prefix for the input.}
...
\vspace{-5.8mm}
\end{wrapfigure}
\mytempedit{If no precondition is considered, then this code can generate an exponential number of paths since any mismatch forces the loop to restart. On the other hand, if a {\em known prefix} precondition is set on the input, then only a single path is generated when exploring the loop. In this way, the engine can focus its exploration on the remaining actually interesting code.}
\end{shaded}

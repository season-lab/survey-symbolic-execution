...
One of the main challenges of symbolic execution is the path explosion problem: a symbolic executor may fork off a new state at every branch of the program, and the total number of states may be thus exponential in the number of branches. Keeping track of a large number of pending branches to be explored, in turn, might impact both running time and space requirements of the symbolic executor. 
...
Many works have attempted to capture similarities across distinct loop iterations or function invocations, e.g., by devising summarization strategies that prevent repeated explorations of a code portion or by inferring invariants that inductively describe properties of a computation. In the reminder of this section we present a number of prominent techniques, which typically compute an under-approximation of the analysis aimed at exploring only a relevant subset of the state space.
...
\cite{GL-ISSTA11} presents a technique that automatically derives partial summarizations for loops. Akin to function calls, a loop summary uses preconditions and postconditions that are dynamically computed  during the symbolic execution by reasoning on the dependencies among loop conditions and symbolic variables. Caching loop summaries not only allows the symbolic engine to avoid redundant executions of the same loop in the same program state, but  makes it also possible to generalize the summary to cover different executions of the same loop under different conditions. Summaries, however, can be generated only for loops that iteratively update symbolic variables across the iterations by adding a constant, non-zero amount.
...
\subsection{Loops (old stuff to be reallocated - where?)} 

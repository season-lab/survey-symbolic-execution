...
While a concrete execution can only underapproximate the analysis of a property under a given workload, symbolic execution can simultaneously explore multiple control flow paths that a program could take under different inputs. This paves the road to sound analyses that can yield strong guarantees on the checked property. Symbolic execution may answer useful questions on concrete programs like: ``does function {\tt foo(x)} always return a positive value for any possible value of {\tt x}?'' The key idea behind symbolic execution is to allow a program to take on ``symbolic'' -- rather than concrete -- input values. Execution is performed by an {\em abstract machine}, which maintains for each explored control flow path: (i) a first-order {\em path formula} that describes the conditions satisfied by the branches taken on that path, and (ii) a {\em symbolic store} that maps variables to symbolic expressions. A model checker, typically based on a SMT solver\mynote{add citation}, is used to verify if there are any violations of property $\phi$ along the explored paths and if paths themselves are realizable, i.e., path formulas are satisfied by some assignment of concrete values to the program's symbolic arguments.

...
Symbolic execution is a powerful program analysis technique first introduced in the mid 70's in the context of software testing (see, e.g.,~{\cite{K-CACM76} and~\cite{H-TSE77}})\mynote{IF: are references appropriate? Should we give more credits? It seems several groups introduced independently this technique}. The basic idea is to allow the program to take on ``symbolic'' -- instead of concrete -- input values. Compared to a concrete execution, where a single path in the control flow graph is taken, symbolic execution aims to simultaneously explore all the potential paths that a program could take if fed with different inputs. Execution is performed by an abstract machine, which maintains for each explored path a mapping from variables to symbolic expressions and a first-order Boolean formula that describes the conditions satisfied by the branches taken on that path.
...
We start with a simple example that highlights many of the fundamental issues discussed in remainder of this article.

...
\item $\sigma_s$ is a {\em symbolic store} that associates variables with symbolic values. 
  ...
  \item $pct$ is {\em path constraint} \mynote{IF: my feeling is that pc does not denote a set. Better notation? Isn't pc a logical formula?} constraints, i.e., a set of assumptions made over the symbols $\alpha_i$ in order to reach $stmt$. Initially $pc$ is empty, and is thus trivially satisfied.

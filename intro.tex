...
Consider the simple C function shown in Figure~\ref{fig:example-1}. The most critical operation is the division performed at line 10. Each of the {\tt int} input values {\tt a}, {\tt b}, and {\tt c} can be assigned with $2^{32}$ distinct values, though only values leading to $x + y + z - 3 = 0$ would make the code crash. Hence, while techniques such as random testing could generate bottomless input tests for this function, it is unlikely that the crash-inducing inputs would be randomly picked up\mynote{Fuzzing?}. 

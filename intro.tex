...
%%%
\item {\em Environment and Third-Party Components}: how does the symbolic engine handle interactions across the software stack? Call to libraries and system code can cause side-effects such as the creation of a file that could later affect the execution and must thus be taken into account. However, evaluating any possible interaction outcome may be unfeasible: it would give rise to a large number of states, while only a fraction of them can \mynote{[D] likely?}actually happen in a non-symbolic scenario.
...
\mytempedit{
Symbolic execution has been the focus of a vast body of literature. As of August 2017, Google Scholar reports 742 articles that include the exact phrase ``symbolic execution'' in the title. Prior to this survey, other authors have contributed technical overviews of the field. Among them, we cite \cite{PV-JSTTT09} and \cite{CS-CACM13}. \cite{CHEN20131758} focuses on the more specific setting of automated test generation. This work provides a comprehensive and updated view of the literature. It complements the technical discussions with a number of running examples and covers in depth recent techniques for key aspects such as memory modelling, environment interaction, path explosion, and constraint solving.
}

...
Symbolic execution has been originally introduced to perform automatic testing of software. As an example, consider the C function shown in Figure~\ref{fig:example-1}. A critical operation performed by this code is the division at line 10. In principle, testing techniques such as random testing can generate bottomless input tests\mynote{IF: what are ''bottomless'' input tests?} for this function, although it is unlikely that the inputs which make this code crash are effectively picked up by these techniques\mynote{Fuzzing?}. Indeed, albeit this code is relatively simple, each of the {\tt int} input values {\tt a}, {\tt b}, and {\tt c} can be assigned with $2^{32}$ distinct values, but only those leading to $x + y + z - 3 = 0$ would make the code crash.
...
In symbolic execution, every global or local variable is associated with a {\em symbol} $\alpha_i$.  At any point of the execution, the symbolic engine maintains:
...
One of the main challenges of symbolic execution is the state space explosion problem. Since symbolic execution forks off a new interpreter at every (undecidable)\mynote{why ``undecidable''?} branch, the total number of interpreters is exponential in the number of branches of the program. The approach taken is to compute an under-approximation of the analysis that only considers a relevant subset of the state space.
...
\begin{itemize}
  ...
  \item {\em Concolic Execution}. Concolic execution [24] can be viewed as a specific form of preconditioned symbolic execution where the precondition is specified by 
...
\begin{itemize}
  ...
  \item {\em Known Prefix}. For instance, we add constraint that the $P$ bytes of {\tt input} are known ($P < N < S$). E.g., fixed header string such as the initial {\em magic code} of a binary input file). Since first $P$ branches and first $P$ iterations are concrete, then state space is $S \cdot 2^{N-P} \cdot 2^M$.
...
\begin{itemize}
  ...
  \item possibly a relevant paper is \cite{S-FMCAD08}

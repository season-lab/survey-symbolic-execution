...
One of the main challenges of symbolic execution is the state space explosion problem. Since symbolic execution forks off a new interpreter at every (undecidable)\mynote{why ``undecidable''?} branch, the total number of interpreters is exponential in the number of branches of the program. A common approach is to compute an under-approximation of the analysis that only considers a relevant subset of the state space.
